...
public class ArticleAction extends BaseAction {
	...
	public String getAllArticle() {
		...
		tempList = this.articleService.show();
		...
		if ((Integer.parseInt(this.number) + 1) == 1) {
			...
			buffer.append("<a href=\"article/getAllArticle.action?number=" + (Integer.parseInt(this.number) - 1) + "\">上一页</a>");
		}
		buffer.append("&nbsp;&nbsp;");
		if (this.maxPage <= (Integer.parseInt(this.number) + 1)) {
			buffer.append("下一页");
		} else {
			buffer.append("<a href=\"article/getAllArticle.action?number=" + (Integer.parseInt(this.number) + 1) + "\">下一页</a>");
		}
		buffer.append("&nbsp;&nbsp;");
		if (this.maxPage <= (Integer.parseInt(this.number) + 1)) {
			buffer.append("尾页");
	...
	public String getArticleById() {
		this.article = this.articleService.checkId(this.id);
		return SUCCESS;
	}
	...
	public String queryArticleByCond() {
		...
		if ("title".equals(this.cond)) {
			list = this.articleService.checkByLikeTitle(this.name);
		}
		if ("contents".equals(this.cond)) {
			list = this.articleService.checkByLikeContents(this.name);
		}
		if ("addtime".equals(this.cond)) {
			list = this.articleService.checkByLikeAddtime(this.name);
		}
		if ("hits".equals(this.cond)) {
			list = this.articleService.checkByLikeHits(this.name);
		}
		return SUCCESS;
	}
	...
	/**
	 * 此程序作者 QQ:709664889 于20170309 012859开发 如有修改请联系本QQ
	 */
